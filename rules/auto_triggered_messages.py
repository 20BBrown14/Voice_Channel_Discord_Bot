...
async def apply(client, message):
  ...
  if(content == 'foos?'):
    ...
    await send_message(client, message, foos_response_array[random.randint(0,len(foos_response_array)-1)])
  elif(content == 'ope'):
    gottem_array = ['gottem', 'gotem', 'gotm', 'gottum', 'gottm', 'gotm', 'gotim', 'gottim', 'oof', 'gotus', 'gottus', '**OOF**', '***OOF***']
    await send_message(client, message, gottem_array[random.randint(0,len(gottem_array)-1)])
  elif(content == 'oof'):
    oof_array = ['**BIG OOF**','you dun goofed', 'Pay some respect, send an F', 'oh snap','_nice job_','Can we get an F in chat?', 'Press F'];
    await send_message(client, message, oof_array[random.randint(0,len(oof_array)-1)])
  elif(content == 'lol'):
    await send_message(client, message, 'lo\nlo\nlol')
